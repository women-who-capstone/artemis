...
class SingleChannel extends React.Component {
	...
	extractMostRecentlyPlayedEpisode() {
		...
		let episodeDates = episodes.map((episode) => new Date(episode.date).getTime());

		let currentEpisodeDate = Math.min(...episodeDates);
		let currentEpisode = episodes.find((episode) => new Date(episode.date).getTime() === currentEpisodeDate);
